...
///
/// 흐름:
///   Submit 클릭 → PreDialogueObject 활성화 → SubmitFinalDecision()
...
public class FinalDecisionUI : MonoBehaviour
{
    ...
    private void OnSubmitClicked()
    {
        ...
        var wrongCards = new List<RoleCard>();
        foreach (var slot in _roleSlots)
        {
            ...
            if (!correct)
                wrongCards.Add(slot.AssignedCard);
        }

        _submitButton.interactable = false;

        if (_preDialogueObject != null) _preDialogueObject.SetActive(true);
        gfc.SubmitFinalDecision(wrongCards.Count == 0);
    }
